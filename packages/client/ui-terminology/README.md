...
| `explainMaxTokens` | `2048` | Completion budget of one explain request; a thinking-capable model spends part of it on reasoning it never shows. |
...
- **Body explanations refresh through the annotation hook** — a vocabulary change repaints settled prose through the same channel that annotates it; the repaint is the cost, and one per verbatim vocabulary change is the bound.
- **One resolved vocabulary per browser** — the resolved vocabulary follows the session the browser fetched against; several sessions open side by side annotate under that one vocabulary until a later refetch resolves another.
...
- **A project watch arms only once its directory exists** — a glossary created externally in a workspace that never held `.dsh/` reaches the vocabulary through a restart or a project-layer `remember`, because reads never create the directory and an absent directory leaves no watcher to arm.
