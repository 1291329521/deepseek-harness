...
- Scope: what the manual-lookup panel does when one passage cannot determine a term, and how the session agent takes over the exploration. No new model tool, no session-protocol change, no host-side repository search.
...
**When the passage cannot determine a term, the panel says so instead of guessing, and offers to hand the question to the session agent, which explains the term from the context it already holds and reads the repository only when that is insufficient; the result can then be folded into the project glossary.**
...
- The agent pays only for what the passage cannot supply. The instruction asks for the existing context first, so a term the agent can already see costs no repository reading.
...
- Host-side repository search before the explain call. Rejected: it sends repository content to whatever route the session selects, and it turns a two-second lookup into a retrieval pipeline.
...
`TerminologyExplainResult`'s success value becomes a discriminated union: `{ kind: 'explained'; explanation: string }` or `{ kind: 'undetermined'; reason: string }`. The union is switched on by `kind` at every consumer, which is this package's own client half. The durable `terminology/explain-request` record is unchanged: it records the request, not the outcome.
...
- The overlay's request state gains `undetermined`, carrying the reason. It renders the reason line, the handoff entry, and the close action.
...
The handoff entry states, beside its label, that it starts one agent turn. A turn is orders of magnitude more expensive than the side-channel call it replaces, so the reader decides with the cost in view rather than discovering it.

### 4.3 Copy ownership

The reason line renders the model's text verbatim as user data. Every string the panel owns — the reason fallback, the handoff label, the cost notice — lives in the locale dictionaries in both locales, reached through the existing `t` seat.
...
The instruction is model-facing English and names the term, the passage it came from, and what makes a good answer. Its exact text is pinned here so the model-visible contract cannot drift silently:
...
- The model emits the marker for a term the passage did determine: the reader pays one agent turn and gets a grounded answer, which is the intended trade.
...
- Unit, policy: the client explain verb maps the undetermined outcome to the new state, and the state carries the reason.
- Component: the overlay renders the reason, the handoff entry, and the cost notice, and hides the add-to-glossary action in this state.
...
- Assembled browser scenario: a scripted undetermined reply opens the panel on the hint and the entry; clicking the entry queues the turn and the transcript shows it; the arrangement keeps the existing glossary and touch assertions in the same scenario.
...
- `packages/client/ui-terminology/src/explain.ts`: prompt instruction, marker parsing, undetermined result.
- `packages/client/ui-terminology/src/types.ts`: the discriminated success value.
- `packages/client/ui-terminology/src/client/overlay-policy.ts`: the undetermined state and the handoff verb.
...
- `packages/client/ui-terminology/README.md` and `README.zh.md`: the state, the handoff, and the attribution limitation.
- `packages/client/ui-terminology/tests/explain.host.spec.ts`, `tests/apply.client.spec.ts`, and the overlay policy spec: the cases in section 8.
...
- The marker is model-authored: a model that never admits ignorance keeps today's behaviour, and a model that over-reports costs the reader an agent turn. The design cannot make the judgement deterministic; it can make the disagreement cheap and visible.
- The queued instruction is attributed to the reader in the transcript, as section 5.3 records.
- The panel gains a third outcome, so the client half's state machine and its tests grow with it.
...
- **Host-side repository search before the explain call.** Rejected: it sends repository content to the routed model on every lookup and duplicates what the agent does better with tools.
...
- Cached reads are not free anywhere: one turn of a session on this machine recorded 219,392 cache-read tokens, so appending a question to a conversation prefix costs far more than the standalone call this design keeps.
