# Web Inline Terminology Implementation Plan

English | [中文](2026-09-16-web-inline-terminology.zh.md)

> **For agentic workers:** REQUIRED SUB-SKILL: Use subagent-driven-development (recommended) or executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax to track progress.

**Goal:** Turn glossary terms inside assistant Markdown into hoverable/focusable/clickable explanation affordances, plus manual "explain the selected text" with a write-back-to-glossary panel, in the DSH Web client.

**架构：** A core rendering seam in `ui-primitives` (`MarkdownAnnotations.split` on `text` nodes + a new `AnnotatedTerm` element) forwarded optionally by `ui-chat` through a `chatAnnotations` service, consumed by one new two-half plugin package `packages/client/ui-terminology`: a Host half owning the two-layer glossary (project file + settings document), a typert remote, and an LLM explain pipeline; a browser half owning the resolver provider, selection/shortcut listeners, explain panel, and settings card.

**技术栈：** TypeScript ESM, Cordis plugin model, React 19 + CSS Modules (`--dsw-*` tokens), Schemastery + Zod, typert Remote, vitest (jsdom where marked), Playwright under `vitest.web.config.ts`.

**规格：** `docs/superpowers/specs/2026-09-16-web-inline-terminology-design.md` / `...design.zh.md` — the spec carries the argumentation; this plan carries the executed steps. Where this plan refines one spec detail, the refinement is stated inline with its reason.

## 全局约束

- Coverage: assistant Markdown only; user questions are out of scope (spec §2.2).
...
- Identity stability is contractual: `MarkdownAnnotations` instances are rebuilt only when the vocabulary changes; `MarkdownText` memoizes settled renders on prop identity.
- When no plugin provides `chatAnnotations`, every rendered byte stays identical to today (`case 'text'` returns the raw string; the `tests/fixtures/markdown-dom` fixtures must pass unmodified).
- Annotations apply to settled renders only; both `StreamingRenderer` contexts keep `annotations: undefined`, mirroring `fileMentions: undefined`.
- No hardcoded tunables in the plugin: every deployment-varying value is a validated `Config` field. Fixed interaction constants (150 ms hover-open delay) are pinned constants, per spec §4.3.
- Client UI copy is locale-owned: every product string goes through the package dictionary (`verify-client-ui-i18n`); `ui-primitives` owns no copy and takes complete label props.
- Bilingual contract for every doc pair (`.md` + `.zh.md` + `.i18n.yaml`), one physical line per paragraph; record pairs with `pnpm run verify-translation-pairing --write <anchor>`.
- Client packages sit inside the per-file 100% coverage gate (`pnpm run test:coverage`); use `/* v8 ignore -- <reason> */` only with a real reason.
- Gates to run at the points marked in the tasks: `pnpm run test:gui`, `DSH_SNAPSHOT=replay pnpm run test:web`, `pnpm run typecheck`, `pnpm run lint`, `pnpm run duplication`, `pnpm run doc-sync`, `pnpm run gen-persistence-catalog`.
- Do not touch the pre-existing red in `docs/superpowers/plans/2026-09-08-task-protocol-p1.md` and `docs/superpowers/specs/2026-09-08-task-protocol-p1-design.md`.
- Comments and JSDoc in English; code comments state contracts, not narration.
- Plan deviations from the spec are refinements, each stated where it happens: (1) spec §6.1 says `node:fs.watch`; this plan uses `chokidar` with `awaitWriteFinish`, the maintained watcher `packages/settings/settings-file` already uses; (2) spec §4.4 says `AssistantNodeView` uses `useMemo(() => annotations(), [annotations])`; this plan calls `annotations()` directly in the render body — the forwarded provider function identity is stable for the plugin's lifetime, so memoizing on it would freeze the resolver forever and never pick up vocabulary changes.
...
PR 4 — LLM explain + durable event:
...
PR 7 — docs: finalize READMEs, subsystem doc note, bilingual Agent Note.
