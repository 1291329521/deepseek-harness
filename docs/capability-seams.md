...
| `ctx.terminology` | `core` | [`client-ui-terminology`](../packages/client/ui-terminology) | - | - | - | Owns the two vocabulary layers, the project-file watcher, the resolved-vocabulary render input, and the manual explain pipeline behind a durable request record. |
